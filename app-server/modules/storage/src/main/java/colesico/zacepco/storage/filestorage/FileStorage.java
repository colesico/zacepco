...
public class FileStorage {
    ...
    @PostConstruct
    public void init() {
        log.info("Initialize file storage '{}'", config.storagePath());
        var path = config.storagePath();
        // Check if the directories do not exist
        if (Files.notExists(path)) {
            try {
                Files.createDirectories(path);
                log.info("File storage directories were missing and have been successfully created.");
            } catch (IOException e) {
                log.error("Failed to create file storage directories: " + e.getMessage());
            }
        }
    }
